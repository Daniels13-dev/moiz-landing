      ...
      <div className="flex-1 pt-12 md:pt-16 px-6 max-w-5xl mx-auto pb-24">
        <header className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
          ...
          {cart.length > 0 && (
            <button
              onClick={clearCart}
              className="text-red-500 font-bold hover:text-red-600 hover:bg-red-50 px-4 py-2 rounded-full transition-colors flex items-center gap-2"
            >
        ...
        <div className="grid lg:grid-cols-12 gap-12">
          <div className="lg:col-span-8">
            <div className="bg-white p-8 md:p-10 rounded-[3rem] shadow-2xl border border-zinc-100 min-h-full flex flex-col">
              <h2 className="text-3xl font-black mb-10 tracking-tight text-zinc-900">
                  ...
                  {cart.length > 0 ? (
                    cart.map((item) => (
                      <motion.div
                        ...
                        className="bg-zinc-50/50 p-4 md:p-6 rounded-[2rem] flex flex-col md:flex-row items-start md:items-center gap-4 md:gap-6 border border-zinc-100 group hover:shadow-md transition-shadow"
                      >
                        <div className="w-20 md:w-24 h-20 md:h-24 bg-white rounded-2xl flex-shrink-0 flex items-center justify-center p-2 overflow-hidden shadow-sm border border-zinc-100">
                          <Image
                            src={item.image}
                            alt={item.name}
                            width={80}
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className="text-xl font-extrabold text-zinc-900 truncate mb-1">
                            {item.name}
                          </h3>
                        ...
                        <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-end">
                          <div className="flex items-center gap-3 bg-white shadow-sm p-1.5 rounded-[1.25rem] border border-zinc-200">
                            <button
                              onClick={() => updateQuantity(item.id, -1)}
                              className="w-10 h-10 flex items-center justify-center rounded-xl bg-white text-zinc-600 hover:bg-[var(--moiz-green)] hover:border-[var(--moiz-green)] hover:text-white transition-all shadow-sm"
                            >
                              <Minus size={16} strokeWidth={3} />
                            </button>
                            <span className="w-6 text-center font-black text-zinc-900 text-lg">
                              {item.quantity}
                            </span>
                            <button
                              onClick={() => updateQuantity(item.id, 1)}
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="text-zinc-500 max-w-xs mx-auto mb-8">
                          Parece que aún no has agregado arena Möiz a tu pedido.
                        </p>
                        <a
                          href="/productos"
