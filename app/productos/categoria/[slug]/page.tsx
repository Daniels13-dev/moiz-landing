...
export async function generateStaticParams() {
  ...
  return categories.map((cat) => ({
    slug: cat.name.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9]+/g, "-"),
  }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const matchedCategory = categories.find(
    (cat) => cat.name.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9]+/g, "-") === slug
  );

  if (!matchedCategory) return { title: "Categoría no encontrada" };

  return {
    title: `Comprar ${matchedCategory.name} para Mascotas | Möiz`,
    description: `Descubre nuestra colección premium de ${matchedCategory.name.toLowerCase()} para mascotas. Envíos a todo el país.`,
  };
}
  ...
  return (
    <main className="bg-[#FAF9F6] min-h-screen flex flex-col selection:bg-[var(--moiz-green)] selection:text-white">
      ...
      <div className="flex-1 pt-8 md:pt-12">
        ...
        <PetShopCatalog initialProducts={initialProducts} initialCategoryName={matchedCategory.name} />
        ...
        <div className="max-w-7xl mx-auto px-6 py-24">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="bg-white p-10 rounded-[3rem] border border-zinc-100">
              ...
              <p className="text-zinc-500 text-sm leading-relaxed">{siteConfig.ui.badges.fastShippingDesc}</p>
            </div>
            <div className="bg-white p-10 rounded-[3rem] border border-zinc-100">
              ...
              <p className="text-zinc-500 text-sm leading-relaxed">{siteConfig.ui.badges.premiumQualityDesc}</p>
            </div>
            <div className="bg-white p-10 rounded-[3rem] border border-zinc-100">
              ...
              <p className="text-zinc-500 text-sm leading-relaxed">{siteConfig.ui.badges.expertSupportDesc}</p>
            </div>
          </div>
        </div>
      </div>
