            ...
            <div className="flex flex-col lg:flex-row gap-20">
              ...
              <aside className="lg:w-1/4 flex flex-col gap-12 border-b lg:border-b-0 lg:border-r border-zinc-100 pb-12 lg:pb-0">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <p className="font-bold text-zinc-900 italic font-playfair">
                    Möiz Editorial Team
                  </p>
                </div>
                <ShareButton />
              ...
              <div
                className="flex-1 prose prose-zinc prose-lg lg:prose-xl max-w-none 
              ...
              >
                {post.content
                  ...
                  .map((para, i, allParas) => {
                    ...
                    if (para.startsWith("# ")) {
                      elements.push(
                        <h1 key={`${i}-h1`} className="text-center md:text-left">
                          {renderFormattedText(para.replace("# ", ""))}
                        </h1>,
                      );
                    } else if (para.startsWith("## ")) {
                      elements.push(
                        <h2 key={`${i}-h2`}>{renderFormattedText(para.replace("## ", ""))}</h2>,
                      );
                    } else if (para.startsWith("### ")) {
                      elements.push(
                        <h3
                          key={`${i}-h3`}
                          className="font-playfair italic text-3xl font-bold mt-16 mb-8 text-zinc-800"
                        >
                          {renderFormattedText(para.replace("### ", ""))}
                        </h3>,
                      );
                      ...
                      elements.push(
                        <blockquote key={`${i}-bq`}>
                          {renderFormattedText(para.replace("> ", "").replace(/"/g, ""))}
                        </blockquote>,
                      );
                      ...
                      elements.push(
                        <div
                          key={`${i}-list`}
                          className="my-16 grid grid-cols-1 md:grid-cols-2 gap-6 pb-8"
                        >
                          {para.split("\n").map((li, j) => (
                            <div
                              key={j}
                              className="group bg-[#FDFDFD] p-8 rounded-[2rem] border border-zinc-50 hover:border-[var(--moiz-green)]/20 transition-all hover:shadow-xl hover:shadow-zinc-100 flex flex-col gap-4"
                            >
                      ...
                      const isFirstPara =
                        i === 1 || (i === 0 && !post.content.trim().startsWith("#"));
                      elements.push(
                        <p
                          key={`${i}-p`}
                          className={
                            isFirstPara
                              ? "text-2xl md:text-3xl font-playfair italic text-zinc-900 leading-[1.6] first-letter:text-8xl first-letter:font-black first-letter:text-[var(--moiz-green)] first-letter:mr-4 first-letter:float-left first-letter:leading-[0.8] mb-12"
                              : "mb-10"
                          }
                        >
                    ...
                    if (i === 1 && featuredProduct) {
                      elements.push(
                        <BlogProductCard key="inline-product" product={featuredProduct} />,
                      );
                    }
                    ...
                    if (
                      i === allParas.length - 1 &&
                      post.slug === "comparativa-bentonita-vs-maiz"
                    ) {
                      elements.push(
